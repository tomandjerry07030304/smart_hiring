...
Write-Host "[OK] Python version: $(python --version)" -ForegroundColor Green
Write-Host ""
...
if (-not $SkipBackend) {
    ...
    $backendBuildScript = Join-Path (Join-Path $PROJECT_ROOT "build_scripts") "build_backend_exe.ps1"
    ...
    if (Test-Path $backendBuildScript) {
        ...
        if (-not (Test-Path $backendDist)) {
            Write-Host "[ERROR] Backend dist directory not found at $backendDist" -ForegroundColor Red
            exit 1
        }
        
        Write-Host "[OK] Backend build completed" -ForegroundColor Green
    } else {
        Write-Host "[WARN] Backend build script not found, skipping backend build" -ForegroundColor Yellow
    }
...
if (-not $SkipFrontend) {
    ...
    if (Test-Path (Join-Path $FRONTEND_DIR "package.json")) {
        ...
        Write-Host "[WARN] Frontend package.json not found, skipping frontend build" -ForegroundColor Yellow
    }
...
if (Test-Path $backendSource) {
    ...
    Write-Host "[OK] Backend copied to build directory" -ForegroundColor Green
} else {
    Write-Host "[WARN] Backend dist not found, installer may not work correctly" -ForegroundColor Yellow
}
...
if ($installers.Count -gt 0) {
    Write-Host "[OK] Installer(s) created successfully:" -ForegroundColor Green
    ...
    Write-Host "[ERROR] Installer not found in expected location" -ForegroundColor Red
...
foreach ($installer in $installers) {
    ...
    Write-Host "[OK] Copied to: $(Join-Path $rootDist $installer.Name)" -ForegroundColor Green
}
