...
Write-Host "âœ“ Python version: $(python --version)" -ForegroundColor Green
Write-Host ""
...
if (-not $SkipBackend) {
    ...
    if (Test-Path $backendBuildScript) {
        ...
        if (-not (Test-Path $backendDist)) {
            Write-Host "âœ— Backend dist directory not found at $backendDist" -ForegroundColor Red
            exit 1
        }
        
        Write-Host "âœ“ Backend build completed" -ForegroundColor Green
    } else {
        Write-Host "âš  Backend build script not found, skipping backend build" -ForegroundColor Yellow
    }
    ...
    if (Test-Path (Join-Path $FRONTEND_DIR "package.json")) {
        ...
        try {
            ...
            if (Test-Path $frontendBuild) {
                ...
                Write-Host "âœ— Frontend build directory not found" -ForegroundColor Red
                exit 1
            }
        }
        finally {
        ...
        Write-Host "âš  Frontend package.json not found, skipping frontend build" -ForegroundColor Yellow
    }
...
if (Test-Path $backendSource) {
    ...
    Write-Host "âœ“ Backend copied to build directory" -ForegroundColor Green
} else {
    Write-Host "âš  Backend dist not found, installer may not work correctly" -ForegroundColor Yellow
}
...
if ($installers.Count -gt 0) {
    Write-Host "âœ“ Installer(s) created successfully:" -ForegroundColor Green
    ...
    Write-Host "âœ— Installer not found in expected location" -ForegroundColor Red
...
foreach ($installer in $installers) {
    ...
    Write-Host "âœ“ Copied to: $(Join-Path $rootDist $installer.Name)" -ForegroundColor Green
}
